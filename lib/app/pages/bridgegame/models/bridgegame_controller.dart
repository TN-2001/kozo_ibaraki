...
class BridgegameController extends ChangeNotifier {
  ...
  */
  ...
  void _init() {
    ...
    for (int i = 0; i < gridHeight; i++) {
      for (int j = 0; j < gridWidth; j++) {
        Elem elem = Elem();
        elem.nodeList = [
          _nodeList[i * (gridWidth + 1) + j],
          _nodeList[i * (gridWidth + 1) + j + 1],
          _nodeList[(i + 1) * (gridWidth + 1) + j + 1],
          _nodeList[(i + 1) * (gridWidth + 1) + j]
        ];
        _elemList.add(elem);
      }
    }
  ...
  void _initCanvas() {
    pcController.resizeCanvas(gridWidth, gridHeight);
  ...
  Future<void> calculation() async {
    ...
    final int npx1 = gridWidth;
    final int npx2 = gridHeight;
    const int nd = 2;

    List<List<int>> zeroOneList =
        List.generate(npx1, (_) => List.filled(npx2, 0));
    for (int n2 = 0; n2 < npx2; n2++) {
      for (int n1 = 0; n1 < npx1; n1++) {
        zeroOneList[n1][n2] =
            _elemList[npx1 * (npx2 - n2 - 1) + n1].isPainted ? 1 : 0;
      }
    }

    // 解析実行
    final result = await compute(runDesFEM70x25, [zeroOneList, powerIndex]);

    // 変位を入手
    for (int n2 = 0; n2 < npx2 + 1; n2++) {
      for (int n1 = 0; n1 < npx1 + 1; n1++) {
        _nodeList[(npx1 + 1) * (npx2 - n2) + n1].becPos = Offset(
            result.$1[((npx1 + 1) * n2 + n1) * nd],
            result.$1[((npx1 + 1) * n2 + n1) * nd + 1]);
      }
    }
    ...
    for (int n2 = 0; n2 < npx2; n2++) {
      for (int n1 = 0; n1 < npx1; n1++) {
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[0] =
            result.$2[n1][n2][0];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[1] =
            result.$2[n1][n2][1];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[2] =
            result.$2[n1][n2][2];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[3] =
            result.$3[n1][n2][0];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[4] =
            result.$3[n1][n2][1];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[5] =
            result.$3[n1][n2][2];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[6] =
            result.$3[n1][n2][3];
        _elemList[npx1 * (npx2 - n2 - 1) + n1].resultList[7] =
            result.$3[n1][n2][4];
      }
    }
  ...
  void selectElem(Offset pos) {
    ...
    for (int i = 0; i < _elemList.length; i++) {
      ...
      if (isCalculation) {
        p0 = _elemList[i].nodeList[0].pos +
            _elemList[i].nodeList[0].becPos * dispScale;
        p1 = _elemList[i].nodeList[1].pos +
            _elemList[i].nodeList[1].becPos * dispScale;
        p2 = _elemList[i].nodeList[2].pos +
            _elemList[i].nodeList[2].becPos * dispScale;
        p3 = _elemList[i].nodeList[3].pos +
            _elemList[i].nodeList[3].becPos * dispScale;
      }
